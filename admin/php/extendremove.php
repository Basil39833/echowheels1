...
<body>                 
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "select * from extendbooking where booking_id='$bid'";
        $res = select_data($sql);
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            $sql3 = "UPDATE extendbooking SET status=2 WHERE booking_id='$product_id'";
            update_data($sql3);
            /*$sql4 = "UPDATE product SET status=0 WHERE product_id='$scooter_id'";
            update_data($sql4);*/
        }
    ...
    Swal.fire({
        ...
        window.location.replace('../extendbookedscooter.php');
    })
    </script>
    <?php
    } else {
